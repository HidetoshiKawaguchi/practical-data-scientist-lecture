...
このリポジトリは、2025年度 神奈川大学「実践的データサイエンス演習」のうち、川口 英俊が担当した講義資料・演習用コード・サンプルコードを公開するためのものです。

資料は教育目的で公開しています。内容は川口個人の見解であり、所属組織の公式見解を表すものではありません。

なお、ディレクトリ名や資料名の番号が `06` から `09` になっているのは、講義全体の通し番号に対応しているためです。`01` から `05` と `10` 以降は別の教員の担当回であり、このリポジトリには含めていません。
...
└── 2025
    ├── 06-course-overview
    │   └── 06-course-overview.pdf
    ├── 07-document-communication
    │   └── 07-document-communication.pdf
    ├── 08-experiment-management
    │   ├── 08-experiment-management.pdf
    │   ├── exercises
    │   └── sample-codes
    └── 09-software-engineering
        ├── 09-software-engineering.pdf
        ├── exercises
        └── sample-codes
```
...
`2025/09-software-engineering/exercises`
...
`2025/09-software-engineering/sample-codes`
...
08回目の演習・サンプルでは UCI Machine Learning Repository の Wine Quality データセットを利用しています。リポジトリ内には以下が含まれています。
...
- 08回目の演習は `requirements.txt` ベースで実行できます。
- 08回目のサンプルコードは `poetry` を前提にした `pyproject.toml` と `poetry.lock` を含みます。
- 09回目の演習も `requirements.txt` ベースです。
- 09回目のサンプルコードには `poetry` ベースの設定が含まれます。
...
1. `2025/06-course-overview/06-course-overview.pdf`
2. `2025/07-document-communication/07-document-communication.pdf`
3. `2025/08-experiment-management/08-experiment-management.pdf`
4. `2025/09-software-engineering/09-software-engineering.pdf`
